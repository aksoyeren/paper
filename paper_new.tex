...
% In this paper a novel structure 
% the library of action is introduced which is an application software developed in Orocos framework.
...
% However, there are a few points that we can use from this simple example:
...
% Throughout this paper we will develop the \textit{put-on-top} example to demonstrate our approach.

% In this section, a symbolic high-level definition of actions is presented.
...
\begin{figure*}
      \centering
      \includegraphics[scale=0.20]{./pdf/action_graph_sec_new2.pdf}
...
\end{figure*}
% First we give a short description of the ontology of actions.
% Then, the symbolic components of actions are introduced.
% The components of this level are as follows:
% Text with citations \cite{RefB} and \cite{RefJ}
% citations \cite{Aein12} and \cite{Aksoy11b}.
...
% Such definition of action is depicted in fig.xxx.
...
% For the put-on example depicted in Fig. \ref{fig:action_graph_sec}, these objects are tool=hand, obj1=box, obj2=box and background=table.

% When these abstract objects are instantiated in the real world, they are represented by one point in Cartesian space located at their center of mass.
% For some actions and objects, however, more that just a single point is needed.
% For example, to cut an object like a banana, we need to know the angle that it makes with the X-axis, to adjust the angle of knife.
% In this case we use models to describe objects.
% In the cutting example, we use a line model which has a parameter that shows the angle of banana.
\begin{table}
% \centering
...
\noalign{\smallskip}\hline
\end{tabular}
% \caption{Abstract relations and their attributes for the put-on action}
\label{tab:relations}
\end{table}
...
% The \emph{sensor} attribute of a relation, determines the type of sensor that is used to detect that relation.
% We use position, tactile, force and vision sensors to obtain the value of a relation.
...
%  the relation between a pair of objects.
% The symbolic entries show the semantic relation between the corresponding pair.
...
% \begin{figure*}
%       \centering
%       \includegraphics[scale=0.2]{./pdf/action_graph_sec.pdf}
%       \caption{ The relation of 
% Two put-on actions are done with different objects and positions.
% The snapshots of real action execution together with graphs annotated to objects are depicted at top.
% The symbolic graphs 
% However, the SEC matrix and graph sequences are the same.}
%       \label{fig:action_graph_sec}
% \end{figure*}
...
% One example is the ability of a human or robot arm to move to a certain pose.

% The proper combination of these primitives form actions 
% While these primitives are necessary to perform actions, they alone don't form a full purposeful actions.
% What forms an action, is the proper combination of them.
...
% The target of the $move$ primitive is either an object in the scene, or just a point in space.
...
% We call such points, abstract positions, and use them in our method.
% 
% In our method, we use two kinds of abstract positions, namely \emph{home} and \emph{goal} positions.
% The example above was a home position, while an example of a goal position is when we push an object on a surface to another point.
% The actual definition of abstract positions depends on the action and position of other objects.
% In Fig. \ref{fig:action_graph_sec} it is shown that the primitive $move(home)$ is used twice in this action.
% \subsection{Complete Definition}
% In the previous sub-sections several components of the high-level definition were introduced.
% Here we sum up the high-level definition with the same example that we started this section with.
% In figure.xx , an example of a put on top action is depicted.
% The snapshots shows the different states of the scene in terms of object relations.
% The corresponding SEC matrix as well as the required primitives to move from one state to the next state are shown.
% % The whole actions is transformed into a finite state machine which shows the states, and the associated primitives.
% 
% This definition is general and abstract,
% however, one needs to relate these symbols with the real world, which is done in the next section.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% SECTION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% This can be calculated simply by comparing the position of robot hand and the height of the background (assuming that we have a flat background).
...
% \begin{table}
% % \centering
% \caption{Summary of object relation detection by different sensors}
% \begin{tabular}{ lllll }
% % \caption{}
% \hline\noalign{\smallskip}
% \# & Abstract Relation &  Before/After Action& During Action\\
% \noalign{\smallskip}\hline\noalign{\smallskip}
% $1$  & R(manipulator,primary obj.) & vison, tactile& position, tactile, vision \\
% % \hline
% $2$  & R(manipulator,secondary obj.)& vison, tactile& position, tactile, vision \\
% % \hline
% $3$  & R(manipulator,background) &  vision,position &  position, vision\\
% % \hline
% $4$  & R(primary obj.,secondary obj.) & vision &  position, force, vision\\
% % \hline
% $5$  & R(primary obj.,background) & vision &  position, force, vision\\
% % \hline
% $6$  & R(secondary obj.,background) & vision &  vision\\
% \noalign{\smallskip}\hline
% \end{tabular}
% % \caption{Abstract relations and their attributes for the put-on action}
% \label{tab:relation_detection}
...
% For example in cutting action, the robot arm keeps a constant force between the knife and banana in Z direction (constrained space),
...
% \begin{figure}[t]
%       \centering
%       \includegraphics[scale=0.28]{./pdf/sec_fsm.pdf}
%       \caption{A mid-level sequencer is implemented by a finite state machine (FSM) for the put-on-top action.
% The action starts if initially inputs (real relations) match the first column of the SEC matrix.
% The transition from state $i$ to $i+1$ occurs when the inputs match the $i$-th column of the SEC matrix.
% Otherwise, the next state is the same as the current state (loop transitions).
% This process is continued until reaching the final state which corresponds to the last column of the SEC matrix.
% }
%       \label{fig:sec_fsm}
% \end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% At the end of this section, we show the specific hardware setup that we used to produce our results.
...
% \subsection{Hardware-related Components}
\begin{figure*}
      \centering
      \includegraphics[scale=1]{./pdf/aladdin_structure_new.pdf}
...
\end{figure*}
% There hardware-related components are the following:


% \subsection{Calculation and Control Components}

% The components which are used for action definition and execution are 
\begin{itemize}
 \item{\textbf{Action}}
...
% It is created by modifying the CartesianGeneratorPos component in \textit{cartesian\_motion\_control} package (reference needed).
% The DMP implementation is according to \cite{Kulvicius2012}.
...
% Our vision system communicates over ROS nodes.
...
% \paragraph{Action}
% The action component holds the parameters of the high-level definition of the action.
% These parameters are the SEC matrix, high-level primitives and the abstract arguments.
% The parameters of the DMP for trajectories are also stored in this component.
% \paragraph{Perception}
% The perception component collects data from all the sensors and determines if relation of objects in the scene are modified.
% It receives the object poses from the vision system (through the ROS-interface component).
% It also monitors other sensors and detects changes in object relations according to section.\ref{sec:real_relations}.
% \paragraph{Coordinator}
% The coordinator component contains a state machine that executes the action.
% The detail of the state machine and execution algorithm will be discussed in \ref{sec:execution_algorithm}.
% Additionally, some function like comparing desired and actual relations is done inside this component.
% 
% \paragraph{Trajectory planner}
% The trajectory planner receives $move()$ and $move_{periodic}()$ primitives and generates the trajectory using the DMPs.
% % It is created by modifying the CartesianGeneratorPos component in \textit{cartesian\_motion\_control} package (reference needed).
% % The DMP implementation is according to \cite{Kulvicius2012}.
% 
% \paragraph{Force Control}
% The force control component implements the $exert()$ primitive.
% The actual contact force is received from the FRI component and using the algorithm of section.\ref{section_primitives},
% the contact force is regulated in the desired direction.
% The output of this component is sent to the control system of the LWR arm, though the FRI component.
% 
% \paragraph{FRI Component}
% The FRI Component communicates with the robot arm through the FRI,
% and provides the necessary input and output data ports to other Orocos components.
% 
% \paragraph{SDH Component}
% The hand primitives (grasp, release and pre-shape) are implemented in this component.
% This component uses the C++ library which is provided to communicate with the Schunk SDH-2 robot hand.
% The tactile sensor data are also processed in this component to detect objects in the hand.
% The parameter $hand\_status$ determines the current status of the hand and is sent to the perception component.
% This parameter takes values : Undefined, Grasped and Empty
% 
% \paragraph{ROS interface}
% % Our vision system communicates over ROS nodes.
% The data ports in Orocos are capable of talking to ROS nodes, using the components of $rtt\_ros\_integration$ stack.
% % \footnote{http://wiki.ros.org/rtt_ros_integration}.
% The ROS interface component is created by using $rtt\_rosnode$ component of this stack to integrate all ROS communications in one component.
% Since our vision system is developed in ROS framework, this component is used to get the information such as 
% objects poses and relations from the vision system and create the necessary data ports to send them to other components.
...
\begin{figure*}
      \centering
      \includegraphics[scale=1]{./pdf/coordinator_fsm2.pdf}
...
% This component receives the object relations from the perception component and decides which primitive should be executed by hardware.
% To do this, it receives the high-level action definition from the action component.
...
%  This includes the SEC matrix, the action primitives in each sub-task and the DMP parameters of each $move()$ or $move_periodic()$ primitive.
...
% A state machine has an initial state, a final state and optionally some other states.
% A state machine is at any time in exactly one of its states.
% Each state has entry, run and exit functions that are called when that state is entered, running and left, respectively.


% The state machine which is used here is conceptually similar to the mid-level sequencer which is used in \cite{aein2013toward}.
...
% The components of the action in three levels can be seen.
...
% The arm and hand primitives are also listed (bottom right).
The symbolic graph sequence (middle) and the SEC matrix (bottom) remain the same for both version.
% The states(S), inputs(U) and outputs (Y) of FSM and their relation to other components are shown.
}
...
\begin{figure*}
      \centering
      \includegraphics[scale=0.5]{./pdf/scenario_result_big.pdf}
